...
! default: 6d cartesian type segment contraction basis set optimized in finite nuclear model
...
! Minimal basis sets are not available
    ...
    subroutine read_gbs(address_basis)
        ...
        loop_n = 1                                                                                 ! count for element in basis set
        element_loop: do
            ...
            angular_loop: do
                ...
                loop_m = loop_m + 1
                if (loop_m == shell_in_element(loop_n)) then
                    exit
                end if
            end do angular_loop
...
! print basis set information, geometry information and calculation settings
    ...
    subroutine input_print (address_basis, address_molecular)
        ...
        write(60,"(A)") '   Cartrsian basis (6D,10F,18G) used.'
        ...
        entire: do
            ...
            ! -----------------------<module SCF>-----------------------
            	...
            	module_SCF: do
                    ...
                    else if (index(keyword,'maxloop') == 1) then
                        if (index(keyword,'=') /= 0) then
                			read(keyword(index(keyword,'=') + 1 : len(trim(keyword))),"(I)",iostat = ios) maxloop
                			if (ios /= 0) call terminate("maxloop setting should be written as 'maxloop=n'")
                		else
                			call terminate("maxloop setting should be written as 'maxloop=n'")
                        end if
                    ...
                    else if (index(keyword,'nudge') == 1) then
                        if (index(keyword,'=') /= 0) then
                			read(keyword(index(keyword,'=') + 1 : len(trim(keyword))),"(I)",iostat = ios) nudge
                			if (ios /= 0) call terminate("Mix parameter should be written as 'nudge=n'")
                		else
                			call terminate("Mix parameter should be written as 'nudge=n'")
                		end if
	                else
...
!-----------------------------------------------------------------------
! juge the self-consistency of computational settings
        ...
        if (subsp < 4) call terminate('inappropriate subsp setting')
        if (nodiis - subsp < 2) call terminate('nodiis should be set larger')
        if (nudge > 1.0 .or. nudge < 0.0) call terminate('nudge shall be in range [0,1]')

...
!------------------------------<1e>------------------------------
! <¦Öi|¦Öj> related
    real(dp),allocatable :: i_j(:,:)                                 ! <¦Öi|¦Öj>
    integer :: evl_count                                             ! number of eigenvalues of <¦Öi|¦Öj> found by dsyevr
    integer,allocatable :: isupp_ev(:)                               ! indices indicating the nonzero elements in Lowdin
    integer :: lwork                                                 ! lwork of <¦Öi|¦Öj> for input of dsyevr
    integer :: liwork                                                ! liwork of <¦Öi|¦Öj> for input of dsyevr
    real(dp),allocatable :: i_j_u(:,:)                               ! upper triangular part of <¦Öi|¦Öj>
    real(dp),allocatable :: Lowdin(:,:)                              ! unitary transformation in Lowdin orthogonalization
    real(dp),allocatable :: evl(:)                                   ! all eigenvalues of <¦Öi|¦Öj> found by dsyevr
    real(dp),allocatable :: work(:)                                  ! work of <¦Öi|¦Öj> for input of dsyevr
    integer,allocatable :: iwork(:)                                  ! iwork of <¦Öi|¦Öj> for input of dsyevr
...
! <¦Öi|p^2|¦Öj> related
    integer :: evl_count_p2                                          ! number of eigenvalues of <¦Öi|p^2|¦Öj> found by dsyevr
    integer,allocatable :: isupp_ev_p2(:)                            ! indices indicating the nonzero elements in AO2p2
    integer :: lwork_p2                                              ! lwork of <¦Öi|p^2|¦Öj> for input of dsyevr
    integer :: liwork_p2                                             ! liwork of <¦Öi|p^2|¦Öj> for input of dsyevr
    real(dp),allocatable :: i_p2_j(:,:)                              ! <¦Öi|p^2|¦Öj>
    real(dp),allocatable :: i_p2_j_u(:,:)                            ! upper triangular part of <¦Öi|p^2|¦Öj>
    real(dp),allocatable :: AO2p2(:,:)                               ! unitary transformation from AO basis to p^2 eigenstate (validated)
    real(dp),allocatable :: evl_p2(:)                                ! all eigenvalues of <¦Öi|p^2|¦Öj> found by dsyevr
    real(dp),allocatable :: work_p2(:)                               ! work of <¦Öi|p^2|¦Öj> for input of dsyevr
    integer,allocatable :: iwork_p2(:)                               ! iwork of <¦Öi|p^2|¦Öj> for input of dsyevr
    complex(dp),allocatable :: exi_T_j(:,:)                          ! extended i_p2_j matrix
    
    
! <¦Öi|V|¦Öj> related
    real(dp),allocatable :: i_V_j(:,:)                               ! <¦Öi|V|¦Öj>
    complex(dp),allocatable :: exi_V_j(:,:)                          ! extended i_V_j matrix
    
    
! DKH2 related
    
    real(dp),allocatable :: i_pxVpx_j(:,:)                           ! <¦Öi|pVp|¦Öj>
    ...
    real(dp),allocatable :: i_px3Vpx_j(:,:)                          ! <¦Öi|pppVp|¦Öj>
    ...
    ! <¦Öi|pxVpy3|¦Öj> = Trans(<¦Öi|py3Vpx|¦Öj>)
...
!------------------<DKH2 HAMILTONIAN>------------------
            ...
            write(60,'(A)') '   The incompleteness of basis set may increase the error in DKH integrals.'
            !-----------------------------------------------
            ! 1e integral calculation
            ...
            ! <¦Öi|p^2|¦Öj> diagonalization
            write(60,'(A)') '   <¦Öi|p^2|¦Öj> diagonalization'
...
! assign value to one electron integral matrices
    subroutine assign_matrices_1e
        ...
        real(dp),allocatable :: exponents_i(:)             ! exponents of |¦Öi>
        real(dp),allocatable :: exponents_j(:)             ! exponents of |¦Öj>
        real(dp),allocatable :: coefficient_i(:)           ! coefficient of |¦Öi>
        real(dp),allocatable :: coefficient_j(:)           ! coefficient of |¦Öj>
        real(dp) :: coordinate_i(3)                        ! coordinate of center of |¦Öi>
        real(dp) :: coordinate_j(3)                        ! coordinate of center of |¦Öj>
        real(dp),allocatable :: coedx_i(:)                 ! coefficient of derivative of x of |¦Öi>
        real(dp),allocatable :: coedy_i(:)                 ! coefficient of derivative of y of |¦Öi>
        real(dp),allocatable :: coedz_i(:)                 ! coefficient of derivative of z of |¦Öi>
        real(dp),allocatable :: coedx_j(:)                 ! coefficient of derivative of x of |¦Öj>
        real(dp),allocatable :: coedy_j(:)                 ! coefficient of derivative of y of |¦Öj>
        real(dp),allocatable :: coedz_j(:)                 ! coefficient of derivative of z of |¦Öj>
        character(len=5) :: chardx_i(2)                    ! x,y,z factor of derivative of x of |¦Öi>
        character(len=5) :: chardy_i(2)                    ! x,y,z factor of derivative of y of |¦Öi>
        character(len=5) :: chardz_i(2)                    ! x,y,z factor of derivative of z of |¦Öi>
        character(len=5) :: chardx_j(2)                    ! x,y,z factor of derivative of x of |¦Öj>
        character(len=5) :: chardy_j(2)                    ! x,y,z factor of derivative of y of |¦Öj>
        character(len=5) :: chardz_j(2)                    ! x,y,z factor of derivative of z of |¦Öj>
        ...
        do while(loop_i <= basis_dimension)
            ...
            ! L start at 1 and is not true angular quantum number
            L_i = atom_basis(molecular(atom_i) % basis_number + shell_i - 1) % angular_quantum_number + 1
            ! M start at 1 and is not true magnetic quantum number
            M_i = loop_i - shell_start_i + 1
            allocate(exponents_i(contraction_i))
            allocate(coefficient_i(contraction_i))
            exponents_i = atom_basis(molecular(atom_i) % basis_number + shell_i - 1) % exponents
            do loop_k=1,contraction_i
                coefficient_i(loop_k) = atom_basis(molecular(atom_i) % basis_number + shell_i - 1) % Ncoefficient(loop_k,M_i)
            end do
            coordinate_i = molecular(atom_i) % nucleus_position                       !(x,y,z)
            allocate(coedx_i(2*contraction_i))
            allocate(coedy_i(2*contraction_i))
            allocate(coedz_i(2*contraction_i))
            !-------------------------------
            ! factor of d(x^m)*exp
            ...
            atom_j = 1
            shell_j = 1
            shell_start_j = loop_j
            do while(loop_j <= basis_dimension)
                ...
                M_j = loop_j - shell_start_j + 1
                allocate(exponents_j(contraction_j))
                allocate(coefficient_j(contraction_j))
                exponents_j = atom_basis(molecular(atom_j) % basis_number + shell_j - 1) % exponents
                do loop_k=1,contraction_j
                    coefficient_j(loop_k) = atom_basis(molecular(atom_j) % basis_number + shell_j - 1) % Ncoefficient(loop_k,M_j)
                end do
                coordinate_j = molecular(atom_j) % nucleus_position                       
                allocate(coedx_j(2*contraction_j))
                allocate(coedy_j(2*contraction_j))
                allocate(coedz_j(2*contraction_j))
                ...
                ! assign <¦Öi|V|¦Öj>
                call assign_1e_V(loop_i,loop_j,contraction_i,contraction_j,coefficient_i,coefficient_j,&
                    AO_xyz_factor(L_i,M_i),AO_xyz_factor(L_j,M_j),exponents_i,exponents_j,coordinate_i,coordinate_j)
                
                if (DKH_order == 2) then
                    ! assign <¦Öi|pVp|¦Öj> and <¦Öi|p^3Vp|¦Öj> and <¦Öi|pVp^3|¦Öj>, totally 27 matrices, 15 matrices will be calculated
                    call assign_1e_pVp('xx',loop_i,loop_j,contraction_i,contraction_j,coedx_i,coedx_j,&
                        chardx_i,chardx_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('yy',loop_i,loop_j,contraction_i,contraction_j,coedy_i,coedy_j,&
                        chardy_i,chardy_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('zz',loop_i,loop_j,contraction_i,contraction_j,coedz_i,coedz_j,&
                        chardz_i,chardz_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('xy',loop_i,loop_j,contraction_i,contraction_j,coedx_i,coedy_j,&
                        chardx_i,chardy_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('yx',loop_i,loop_j,contraction_i,contraction_j,coedy_i,coedx_j,&
                        chardy_i,chardx_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('xz',loop_i,loop_j,contraction_i,contraction_j,coedx_i,coedz_j,&
                        chardx_i,chardz_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('zx',loop_i,loop_j,contraction_i,contraction_j,coedz_i,coedx_j,&
                        chardz_i,chardx_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('yz',loop_i,loop_j,contraction_i,contraction_j,coedy_i,coedz_j,&
                        chardy_i,chardz_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                    call assign_1e_pVp('zy',loop_i,loop_j,contraction_i,contraction_j,coedz_i,coedy_j,&
                        chardz_i,chardy_j,exponents_i,exponents_j,coordinate_i,coordinate_j)
                end if
                !-------------<basis overlap integral>-------------
                do loop_k = 1, contraction_i
                    do loop_m = 1, contraction_j
                        integral_overlap = Gaussian_Product_Integral(coefficient_i(loop_k)*coefficient_j(loop_m),&
                ...
                if (isnan(i_j(loop_i,loop_j))) call terminate('calculation of <¦Öi|¦Öj> failure, NaN detected.')
                !-------------<P2 integral>-------------
                do loop_k = 1, 2*contraction_i
                    do loop_m = 1, 2*contraction_j
                        if (mod(loop_k,2) == 0 .and. mod(loop_m,2) == 0) then
                            integral_dx = Gaussian_Product_Integral(coedx_i(loop_k)*coedx_j(loop_m),&
                        ...
                        i_p2_j(loop_i,loop_j) = i_p2_j(loop_i,loop_j) + integral_dx
                    end do
                end do 
                do loop_k = 1, 2*contraction_i
                    do loop_m = 1, 2*contraction_j
                        if (mod(loop_k,2) == 0 .and. mod(loop_m,2) == 0) then
                            integral_dy = Gaussian_Product_Integral(coedy_i(loop_k)*coedy_j(loop_m),&
                        ...
                        i_p2_j(loop_i,loop_j) = i_p2_j(loop_i,loop_j) + integral_dy
                    end do
                end do
                do loop_k = 1, 2*contraction_i
                    do loop_m = 1, 2*contraction_j
                        if (mod(loop_k,2) == 0 .and. mod(loop_m,2) == 0) then
                            integral_dz = Gaussian_Product_Integral(coedz_i(loop_k)*coedz_j(loop_m),&
                        ...
                        i_p2_j(loop_i,loop_j) = i_p2_j(loop_i,loop_j) + integral_dz
                    end do
                end do
                if (isnan(i_p2_j(loop_i,loop_j))) call terminate('calculation of <¦Öi|p^2|¦Öj> failure, NaN detected.')
                ...
                deallocate(coedz_j)
                loop_j = loop_j + 1
                if (loop_j - shell_start_j >= (L_j + 1) * L_j / 2) then
                    shell_j = shell_j + 1
                    shell_start_j = loop_j
                end if
            end do
        ...
        ! consider the relation p = -iD, p3Vp and pVp3 nochange, p2 and pVp change sign
        if (SRTP_type) then
...
!-----------------------------------------------------------------------
! assign values to matrix <¦Öi|V|¦Öj>
    subroutine assign_1e_V(loop_i,loop_j,contr_i,contr_j,coe_i,coe_j,fac_i,fac_j,expo_i,expo_j,cod_i,cod_j)
        implicit none
        integer,intent(in) :: loop_i                     ! i^th orbital passed form subroutine assign_1e_p2
        integer,intent(in) :: loop_j                     ! j^th orbital passed form subroutine assign_1e_p2
        integer,intent(in) :: contr_i
        integer,intent(in) :: contr_j
        integer :: bloop_i,bloop_j,bloop_pot             ! bloop is loop variables only for subroutine assign_1e_pVp
        ...
        do while(bloop_pot <= atom_count)
            ...
            do bloop_i = 1, contr_i
                do bloop_j = 1, contr_j
                    ...
                    if (isnan(integral_pot)) call terminate('calculation of <¦Öi|V|¦Öj> failure, NaN detected.')
                end do
            end do
            i_V_j(loop_i,loop_j) = i_V_j(loop_i,loop_j) + integral_pot
            bloop_pot = bloop_pot + 1
        end do
    end subroutine assign_1e_V
    
!-----------------------------------------------------------------------
! assign values to matrix <¦Öi|pVp|¦Öj> (9 matrices)
!  pxVpx pyVpy pzVpz pxVpy pyVpx pxVpz pzVpx pyVpz pzVpy
    subroutine assign_1e_pVp(char_der,loop_i,loop_j,contr_i,contr_j,coe_i,coe_j,fac_i,fac_j,expo_i,expo_j,cod_i,cod_j)
        integer,intent(in) :: loop_i                     ! i^th orbital passed form subroutine assign_1e_p2
        integer,intent(in) :: loop_j                     ! j^th orbital passed form subroutine assign_1e_p2
        integer,intent(in) :: contr_i
        integer,intent(in) :: contr_j
        integer :: sloop_i,sloop_j,sloop_pot             ! sloop is loop variables only for subroutine assign_1e_pVp
        ...
        if (char_der == 'xx') then
            code_der = 1
        else if (char_der == 'yy') then
            code_der = 2
        else if (char_der == 'zz') then
            code_der = 3
        else if (char_der == 'xy') then
            code_der = 4
        else if (char_der == 'yx') then
            code_der = 5
        else if (char_der == 'xz') then
            code_der = 6
        else if (char_der == 'zx') then
            code_der = 7
        else if (char_der == 'yz') then
            code_der = 8
        else if (char_der == 'zy') then
            code_der = 9
        else
            call terminate('assign_1e_pVp is called incorrectly, unknown char_der')
        end if
        sloop_pot = 1
        do while(sloop_pot <= atom_count)
            ! use the relation <¦Öi|pVp'|¦Öj> = <¦Öj|p'Vp|¦Öi>
            if (code_der == 5) then
                i_pyVpx_j(loop_j,loop_i) = i_pxVpy_j(loop_i,loop_j)
                exit
            else if (code_der == 7) then
                i_pzVpx_j(loop_j,loop_i) = i_pxVpz_j(loop_i,loop_j)
                exit
            else if (code_der == 9) then
                i_pzVpy_j(loop_j,loop_i) = i_pyVpz_j(loop_i,loop_j)
                exit
            end if
            integral_pot = 0.0_dp
            ...
            do sloop_i = 1, 2*contr_i
                do sloop_j = 1, 2*contr_j
                    ...
                    if (isnan(integral_pot)) call terminate('calculation of <¦Öi|pVp|¦Öj> failure, NaN detected.')
                end do
            end do
            if (code_der == 1) then
                i_pxVpx_j(loop_i,loop_j) = i_pxVpx_j(loop_i,loop_j) + integral_pot
            else if (code_der == 2) then
                i_pyVpy_j(loop_i,loop_j) = i_pyVpy_j(loop_i,loop_j) + integral_pot
            else if (code_der == 3) then
                i_pzVpz_j(loop_i,loop_j) = i_pzVpz_j(loop_i,loop_j) + integral_pot
            else if (code_der == 4) then
                i_pxVpy_j(loop_i,loop_j) = i_pxVpy_j(loop_i,loop_j) + integral_pot
            else if (code_der == 6) then
                i_pxVpz_j(loop_i,loop_j) = i_pxVpz_j(loop_i,loop_j) + integral_pot
            else if (code_der == 8) then
                i_pyVpz_j(loop_i,loop_j) = i_pyVpz_j(loop_i,loop_j) + integral_pot
            end if
            sloop_pot = sloop_pot + 1
        end do
        if (SRTP_type) call assign_1e_pppVp(code_der,loop_i,loop_j,contr_i,contr_j,coe_i,coe_j,fac_i,fac_j,expo_i,expo_j,cod_i,cod_j)
    end subroutine assign_1e_pVp
    
!-----------------------------------------------------------------------
! assign values to matrix <¦Öi|p^3Vp|¦Öj> (9 matrices)
!  px3Vpx py3Vpy pz3Vpz px3Vpy py3Vpx px3Vpz pz3Vpx py3Vpz pz3Vpy
    subroutine assign_1e_pppVp(code_der,loop_i,loop_j,contr_i,contr_j,coe_i,coe_j,fac_i,fac_j,expo_i,expo_j,cod_i,cod_j)
    	integer,intent(in) :: code_der                   ! which pVp matrix is assignning (e.g. 2 for py3Vpy)
    	integer,intent(in) :: loop_i                     ! i^th orbital passed form subroutine assign_1e_p2
        integer,intent(in) :: loop_j                     ! j^th orbital passed form subroutine assign_1e_p2
        integer,intent(in) :: contr_i
        integer,intent(in) :: contr_j
        integer :: tloop_i,tloop_j,tloop_pot             ! tloop is loop variables only for subroutine assign_1e_pVp
        ...
        character(len = *),intent(in) :: fac_j(2)        ! xyz factor of j^th orbital
        character(len = 1) :: ele_der                    ! find the derivative of p|¦Öi> respect to x/y/z
        if (code_der == 1 .or. code_der == 4 .or. code_der == 6) then
            ele_der = 'x'
        else if (code_der == 2 .or. code_der == 5 .or. code_der == 8) then
            ele_der = 'y'
        else if (code_der == 3 .or. code_der == 7 .or. code_der == 9) then
            ele_der = 'z'
        else
            call terminate('assign_1e_pppVp is called incorrectly, unknown code_der')
        end if
        ...
        do while(tloop_pot <= atom_count)
            ...
            do tloop_i = 1, 8*contr_i
                do tloop_j = 1, 2*contr_j
                    ...
                    if (isnan(integral_pot)) call terminate('calculation of <¦Öi|p^3Vp|¦Öj> failure, NaN detected.')
                end do
            end do
            if (code_der == 1) i_px3Vpx_j(loop_i,loop_j) = i_px3Vpx_j(loop_i,loop_j) + integral_pot
            if (code_der == 2) i_py3Vpy_j(loop_i,loop_j) = i_py3Vpy_j(loop_i,loop_j) + integral_pot
            if (code_der == 3) i_pz3Vpz_j(loop_i,loop_j) = i_pz3Vpz_j(loop_i,loop_j) + integral_pot
            if (code_der == 4) i_px3Vpy_j(loop_i,loop_j) = i_px3Vpy_j(loop_i,loop_j) + integral_pot
            if (code_der == 5) i_py3Vpx_j(loop_i,loop_j) = i_py3Vpx_j(loop_i,loop_j) + integral_pot
            if (code_der == 6) i_px3Vpz_j(loop_i,loop_j) = i_px3Vpz_j(loop_i,loop_j) + integral_pot
            if (code_der == 7) i_pz3Vpx_j(loop_i,loop_j) = i_pz3Vpx_j(loop_i,loop_j) + integral_pot
            if (code_der == 8) i_py3Vpz_j(loop_i,loop_j) = i_py3Vpz_j(loop_i,loop_j) + integral_pot
            if (code_der == 9) i_pz3Vpy_j(loop_i,loop_j) = i_pz3Vpy_j(loop_i,loop_j) + integral_pot
	    	tloop_pot = tloop_pot + 1
        end do
    end subroutine assign_1e_pppVp

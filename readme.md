...
> It is not recommended to build and use TRESC on Windows

1. Make sure the build tools are available on your station:
[Visual Studio Build Tools](https://visualstudio.microsoft.com/),
[Intel oneAPI HPC Toolkit](
  https://www.intel.com/content/www/us/en/developer/tools/oneapi/toolkits.html)
and [Cmake](https://cmake.org/), ONEAPI_ROOT is in environment variables:
...
then<br>
`cd build && ninja`<br>
make sure LIBXC_ROOT is in environment variables:
`LIBXC_ROOT = \path\to\Libxc`
3. Change directory to TRESC root and build TRESC by command:<br>
`cmake -B "build\\release" -G Ninja -DCMAKE_BUILD_TYPE=Release`<br>
then<br>
`cmake --build "build\\release"`<br>
finally, add `...\TRESC\build` to $path and add `...\TRESC` to $TRESC.
...
series of Becke grid data binary files (via `TRESC -2c`), and then visualise the

...
! calculation settings (with default)
  ...
  real(dp)        :: diisdamp_
  real(dp)        :: prtlev      = 0.1    ! minimum AO coefficient print output
  ...
  integer         :: fx_id       = -1     ! exchange functional ID
  integer         :: fc_id       = -1     ! correlation functional ID
  integer         :: fxc_id      = -1     ! exchange-correlation functional ID
...
!> generate output (.esc) file
  subroutine generate_output()
    ...
    write(60,"(A)") "  Intel MKL: numerical linear algebra operations"
    write(60,"(A)") "  OpenMP: multithreaded parallel calculation"
    write(60,"(A)") "  Libxc: exchange and correlation functionals"
    write(60,*)
...
!------------------------------------------------------------
!> swap 2 real(dp) in place
  pure subroutine real_swap(x, y)
    implicit none
...
!------------------------------------------------------------
!> swap 2 complex(dp) in place
  pure subroutine cmplx_swap(x, y)
    implicit none
    ...
    x = y
    y = tmp
  end subroutine cmplx_swap

!------------------------------------------------------------
!> swap 2 integer in place
  pure subroutine int_swap(x, y)
    implicit none
...
!------------------------------------------------------------
!> sort a real(dp) array OUT OF PLACE
  pure subroutine real_sort(arr, sorted, sortindex)
    implicit none
    real(dp), intent(in)            :: arr(:)       ! origin array to be sort
    real(dp), intent(out)           :: sorted(:)    ! sorted array
    integer, intent(out), optional  :: sortindex(:) ! sorted index array
    integer                         :: ii
    sorted(1:size(arr)) = arr
    if (present(sortindex)) then
      forall (ii = 1:size(arr)) sortindex(ii) = ii
      call quicksort(sorted(1:size(arr)), 1, size(arr), sortindex(1:size(arr)))
      return
    else
      call quicksort(sorted(1:size(arr)), 1, size(arr))
      return
    end if
  end subroutine real_sort

!------------------------------------------------------------
!> sort a complex(dp) array by its amplitudes OUT OF PLACE
  pure subroutine cmplx_sort(arr, sorted, sortindex)
    implicit none
    complex(dp), intent(in)         :: arr(:)       ! origin array to be sort
    real(dp),intent(out)            :: sorted(:)    ! sorted array
    integer, intent(out), optional  :: sortindex(:) ! sorted index array
    integer                         :: ii
    sorted(1:size(arr)) = abs(arr)
    if (present(sortindex)) then
      forall (ii = 1:size(arr)) sortindex(ii) = ii
      call quicksort(sorted(1:size(arr)), 1, size(arr), sortindex(1:size(arr)))
      return
  ...
  end subroutine cmplx_sort

!------------------------------------------------------------
!> quick sort of a real(dp) array IN PLACE
!!
!! if present(arrindex, integer array with same size), sort it in the same way
!!
!! arrindex can be in any order (for secondary sorting), but if it's the first
!!
!! sort, assign it in ascending order from 1 to size(arr) before calling
  recursive pure subroutine quicksort(arr, left, right, arrindex)
    implicit none
    real(dp), intent(inout)          :: arr(:)
    integer, intent(in), optional    :: left, right
    integer, intent(inout), optional :: arrindex(:)
    integer                          :: pivot, l, m, r, len
    integer                          :: ii, jj
    real(dp)                         :: insertkey, vpivot
    integer                          :: insertindex
    ...
    len = r - l + 1
    if (len <= 0) return
    if (present(arrindex)) then
      !-----------------<insertion sort>------------------
      if (len <= 16) then
        do ii = l+1, r
          insertkey = arr(ii)
          insertindex = arrindex(ii)
          jj = ii - 1
          do while (jj >= l .and. arr(jj) > insertkey)
            arr(jj+1) = arr(jj)
            arrindex(jj+1) = arrindex(jj)
            jj = jj - 1
          end do
          arr(jj+1) = insertkey
          arrindex(jj+1) = insertindex
        end do
        return
      end if
      !-----------------<insertion sort>------------------
      !---------<quick sort with Hoare partition>---------
      ! median-of-3, its boundaries arr(l) and arr(r) ensure that
      ! the pointers stay within the sorting range.
      m = int(0.5*(l+r))
      if (arr(l) > arr(m)) then
        call real_swap(arr(l), arr(m))
        call int_swap(arrindex(l), arrindex(m))
      end if
      if (arr(m) > arr(r)) then
        call real_swap(arr(m), arr(r))
        call int_swap(arrindex(m), arrindex(r))
      end if
      if (arr(l) > arr(m)) then
        call real_swap(arr(l), arr(m))
        call int_swap(arrindex(l), arrindex(m))
      end if
      vpivot = arr(m) ! pivot set to a(m)
      ii = l ! left pointer
      jj = r ! right pointer
      do
        do while (arr(ii) < vpivot)
          ii = ii + 1
        end do
        do while (arr(jj) > vpivot)
          jj = jj - 1
        end do
        if (ii >= jj) then
          pivot = jj    ! take right pointer as pivot
          exit
        end if
        call real_swap(arr(ii), arr(jj))
        call int_swap(arrindex(ii), arrindex(jj))
        ii = ii + 1
        jj = jj - 1
      end do
      !---------<quick sort with Hoare partition>---------
      ! |     |     |     |     |     |     |     |     |
      ! v     v     v     v     v     v     v     v     v
      ! quick sort gives 2 intervals: [l..pivot] and [pivot+1..r]
      ! take them to recursion tree
      if (pivot > l) call quicksort(arr, l, pivot, arrindex) 
      if (pivot+1 < r) call quicksort(arr, pivot+1, r, arrindex)
    else
      !-----------------<insertion sort>------------------
      if (len <= 16) then
        do ii = l+1, r
      ...
      ! the pointers stay within the sorting range.
      m = int(0.5*(l+r))
      if (arr(l) > arr(m)) call real_swap(arr(l), arr(m))
      if (arr(m) > arr(r)) call real_swap(arr(m), arr(r))
      if (arr(l) > arr(m)) call real_swap(arr(l), arr(m))
      vpivot = arr(m) ! pivot set to a(m)
      ii = l ! left pointer
      jj = r ! right pointer
      ...
      ! quick sort gives 2 intervals: [l..pivot] and [pivot+1..r]
      ! take them to recursion tree
      if (pivot > l) call quicksort(arr, l, pivot) 
